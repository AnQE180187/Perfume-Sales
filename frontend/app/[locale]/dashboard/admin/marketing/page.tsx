        ...
        <AuthGuard allowedRoles={['admin']}>
            <main className="p-8">
                <header className="mb-12">
                    <h1 className="text-4xl font-heading gold-gradient mb-2 uppercase tracking-tighter">Growth Engine</h1>
                    <p className="text-muted-foreground font-body text-sm uppercase tracking-widest">Campaigns & Audience Intelligence</p>
                </header>

                <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
                    {[
                        { label: 'Subscribers', value: '8.2k', icon: Users },
                        { label: 'Open Rate', value: '24.5%', icon: Mail },
                        { label: 'Conversion', value: '3.8%', icon: TrendingUp },
                        { label: 'Ad ROI', value: '4.2x', icon: Zap }
                    ].map((stat, i) => (
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    <div className="lg:col-span-2 glass rounded-[2.5rem] border-border p-8">
                        <h2 className="font-heading text-lg uppercase tracking-widest mb-6">Active Campaigns</h2>
                                ...
                                <div key={i} className="p-6 rounded-3xl bg-secondary/20 border border-border flex items-center justify-between">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-[10px] text-muted-foreground uppercase tracking-[0.2em]">Ends in 4 days</p>
                                        </div>
                                    </div>
                                    <span className="bg-emerald-500/10 text-emerald-500 text-[8px] uppercase tracking-[0.2em] font-bold px-3 py-1 rounded-full border border-emerald-500/20">Active</span>
                                </div>
                            ))}
                        </div>
                    </div>
