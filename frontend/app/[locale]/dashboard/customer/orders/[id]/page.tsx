...
const STATUS_CONFIG: Record<string, { label: string; color: string }> = {
    ...
    PROCESSING: { label: 'Đang chuẩn bị', color: 'bg-purple-500/10 text-purple-600 border-purple-500/20' },
    SHIPPED: { label: 'Đang giao hàng', color: 'bg-orange-500/10 text-orange-600 border-orange-500/20' },
        ...
        return (
            <AuthGuard allowedRoles={['customer', 'staff', 'admin']}>
                <div className="py-20 text-center">
                    <p className="text-stone-500 dark:text-stone-400 mb-4">Không tìm thấy đơn hàng</p>
                    <Link href="/dashboard/customer/orders" className="text-gold hover:underline font-bold">
            ...
            <div className="flex flex-col gap-10 py-10 px-8">
                <header>
                    ...
                    <p className="text-[10px] text-stone-500 uppercase tracking-[.4em] font-bold">
                        {new Date(order.createdAt!).toLocaleDateString('vi-VN', { dateStyle: 'full' })}
                    </p>
                </header>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="space-y-8">
                        <div className="glass bg-white dark:bg-zinc-900 rounded-[3rem] p-8 border border-stone-100 dark:border-white/5">
                            ...
                            <span className={`inline-flex items-center gap-2 px-4 py-2 rounded-full text-xs font-bold border ${style.color}`}>
                                {style.label}
                            </span>
                            <p className="mt-6 text-[10px] text-stone-400 uppercase tracking-wider">
                                Thanh toán: {order.paymentStatus === 'PAID' ? 'Đã thanh toán' : order.paymentStatus === 'PENDING' ? 'Chờ thanh toán' : order.paymentStatus}
                            </p>
                        ...
                        {shipments.length > 0 && (
                            <div className="glass bg-white dark:bg-zinc-900 rounded-[3rem] p-8 border border-stone-100 dark:border-white/5">
                                ...
                                <div className="space-y-4">
                                    {shipments.map((s) => (
                                        <div key={s.id} className="p-4 rounded-2xl bg-stone-50 dark:bg-zinc-800 border border-stone-100 dark:border-white/5">
