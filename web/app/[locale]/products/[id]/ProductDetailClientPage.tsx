...
export default function ProductDetailClientPage() {
    ...
    if (loading) {
        return <div className="min-h-screen bg-stone-50 dark:bg-zinc-950 flex items-center justify-center">Loading product...</div>;
    }
            ...
            <main className="container mx-auto px-6 py-32">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 xl:gap-24">
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="grid grid-cols-3 gap-6">
                            {product.images.slice(1, 4).map((image) => (
                                <div key={image.url} className="relative aspect-square rounded-3xl overflow-hidden cursor-pointer border-2 border-transparent hover:border-accent transition-all bg-white dark:bg-zinc-900 shadow-sm">
                                    <Image src={image.url} alt="Preview" fill className="object-cover opacity-60 hover:opacity-100 transition-opacity" />
                                </div>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col">
                        <div className="mb-8">
                            <span className="inline-block px-3 py-1 bg-accent/10 rounded-full text-[10px] font-bold tracking-wider uppercase text-accent mb-4">{product.category.name}</span>
                        ...
                        <div className="flex flex-col sm:flex-row gap-4 mb-16">
                            <button
                                ...
                                className="flex-1 bg-luxury-black dark:bg-accent text-white py-5 rounded-full font-bold tracking-widest uppercase flex items-center justify-center gap-3 hover:bg-stone-800 dark:hover:bg-accent/80 transition-all shadow-xl"
                            >
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 pt-12 border-t border-stone-200 dark:border-white/10 transition-colors">
                            <div className="flex gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs font-medium text-luxury-black dark:text-white transition-colors">{topNotes}</p>
                                </div>
                            </div>
                            <div className="flex gap-4">
                                <div className="w-10 h-10 rounded-full bg-white dark:bg-zinc-900 shadow-sm flex items-center justify-center text-accent transition-colors">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs font-medium text-luxury-black dark:text-white transition-colors">{heartNotes}</p>
                                </div>
                            </div>
                            <div className="flex gap-4">
                                <div className="w-10 h-10 rounded-full bg-white dark:bg-zinc-900 shadow-sm flex items-center justify-center text-accent transition-colors">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs font-medium text-luxury-black dark:text-white transition-colors">{baseNotes}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        {relatedProducts.map((relatedProduct) => (
                            ...
                            >
                                <Link href={`/products/${relatedProduct.id}`}>
                                    ...
                                    <p className="text-[9px] text-stone-500 dark:text-stone-400 font-bold tracking-widest uppercase mb-1 transition-colors">{relatedProduct.category.name}</p>
                                    <h4 className="text-lg font-serif text-luxury-black dark:text-white transition-colors">{relatedProduct.name}</h4>
                                    <p className="text-xs font-medium text-accent mt-2 transition-colors">
