...
export class PromotionsService {
    ...
    async validate(dto: ValidatePromotionDto, userId?: string) {
        ...
        if (now < promo.startDate || now > promo.endDate) {
            throw new BadRequestException('Promotion code has expired or is not yet active');
        }
        ...
        if (promo.minOrderAmount !== null && dto.amount < promo.minOrderAmount) {
            throw new BadRequestException(`Minimum order amount of ${promo.minOrderAmount} required for this code`);
        }
