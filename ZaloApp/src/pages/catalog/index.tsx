...
export default function CatalogPage() {
  ...
  const fetchProducts = async (pageNumber: number) => {
    try {
      ...
      if (pageNumber === 1) {
        ...
        setProducts((prev) => {
          // Avoid duplicates in case of React Strict Mode double-firing
  ...
  return (
    <div className="min-h-full bg-section flex flex-col" style={{ background: '#FAF8F5' }}>
      <ProductFilter />
      <HorizontalDivider />
      ...
      <div className="px-4 py-4">
        ...
        <div ref={loaderRef} className="w-full flex justify-center py-6">
          ...
          {!hasMore && products.length > 0 && (
            <span className="text-xs text-subtitle">Đã xem hết bộ sưu tập</span>
          )}
        </div>
      </div>
    </div>
